  ...
  let embed = CreateEmbed::new()
    ...
    .field(
      ...
      if rating != 0 as u32 {
        ...
        format!("User didn't participate in any contest")
      },
      false,
    )
...
fn get_standing_string(lockout: &Duel) -> String {
  ...
  for (pos, i) in indices.iter().enumerate() {
    ...
    let current = format!(
      "{position} {user_link} {points} points\n",
      position = pos_string,
      user_link = create_href(
        format!(
  ...
  for (i, problem) in problems_arr.iter().enumerate() {
    ...
    if problems_point[i] != 0 {
      let problem_url = format!(
        ...
        index = problem.index,
        contestid = problem.contestId.unwrap()
      );
...
fn get_time_left_string(lockout: &Duel) -> String {
  ...
  let time_so_far = SystemTime::now()
    .duration_since(lockout.begin_time)
    .unwrap();
...
pub async fn edit_to_message(ctx: &Context, embed: CreateEmbed, mut message: Message) {
  let edit_message = EditMessage::new().embed(embed);
  let _ = message.edit(&ctx, edit_message).await;
}

pub async fn edit_to_lockout_status(
  ctx: &Context,
  lockout: &Duel,
  mut message: Message,
  show_problem_set: bool,
) {
  let embed = create_lockout_status_embed(&lockout, show_problem_set);
  let edit_message = EditMessage::new().embed(embed);
  let _ = message.edit(&ctx, edit_message).await;
...
pub fn create_problem_message(
  ...
  let title = format!(
    "{index}. {name}",
    index = problem.index.to_uppercase(),
    name = problem.name
  );
